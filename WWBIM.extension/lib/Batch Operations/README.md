...
### 7. fill_dimensions.py
Заполнение параметров размеров элемента из системных параметров Revit.

**Целевые параметры (7 шт.):**
- `ADSK_Размер_Объём` - объём элемента (м³)
- `ADSK_Размер_Длина` - длина элемента (мм)
- `ADSK_Размер_Ширина` - ширина элемента (мм)
- `ADSK_Размер_Высота` - высота элемента (мм)
- `ADSK_Размер_Толщина` - толщина элемента (мм)
- `ADSK_Площадь` - площадь элемента (м²)
- `ADSK_Размер_Диаметр` - диаметр элемента (мм)

**Источник данных:** Системные параметры Revit (BuiltInParameter) + вычисление габаритов

**Соответствие параметров и категорий:**
| ADSK параметр | Источник | Категории |
|---|---|---|
| ADSK_Размер_Объём | HOST_VOLUME_COMPUTED, STRUCTURAL_VOLUME, RBS_PIPE_VOLUME_PARAM, RBS_DUCT_VOLUME_PARAM | Стены, Перекрытия, Крыши, Потолки, Фундаменты, Колонны, Балки, Трубы, Воздуховоды |
| ADSK_Размер_Длина | CURVE_ELEM_LENGTH, STRUCTURAL_FRAME_CUT_LENGTH, INSTANCE_LENGTH_PARAM, RBS_PIPE_LENGTH_PARAM, RBS_DUCT_LENGTH_PARAM, RBS_CABLETRAY_LENGTH_PARAM, RBS_CONDUIT_LENGTH_PARAM | Стены, Пандусы, Перемычки, Балки, Колонны, Фундаменты, Трубы, Воздуховоды, Лотки, Кабель-каналы |
| ADSK_Размер_Ширина | WALL_USER_WIDTH_PARAM, FLOOR_ATTR_THICKNESS_PARAM, CEILING_THICKNESS, STRUCTURAL_FOUNDATION_THICKNESS, FAMILY_WIDTH_PARAM, RBS_DUCT_WIDTH_PARAM, RBS_CABLETRAY_WIDTH_PARAM | Стены, Перекрытия, Потолки, Фундаменты, Двери, Окна, Мебель, Воздуховоды, Лотки |
| ADSK_Размер_Высота | FAMILY_HEIGHT_PARAM, RBS_DUCT_HEIGHT_PARAM, RBS_CABLETRAY_HEIGHT_PARAM | Двери, Окна, Мебель, Оборудование, Воздуховоды, Лотки, Колонны, Балки, Фундаменты |
| ADSK_Размер_Толщина | WALL_USER_WIDTH_PARAM, FLOOR_ATTR_THICKNESS_PARAM, CEILING_THICKNESS, STRUCTURAL_FOUNDATION_THICKNESS, FAMILY_DEPTH_PARAM | Стены, Перекрытия, Потолки, Фундаменты, Двери, Окна, Колонны, Балки |
| ADSK_Площадь | HOST_AREA_COMPUTED, STRUCTURAL_AREA | Стены, Перекрытия, Крыши, Потолки, Колонны, Балки |
| ADSK_Размер_Диаметр | RBS_PIPE_DIAMETER_PARAM, RBS_PIPE_OUTER_DIAMETER_PARAM, RBS_DUCT_DIAMETER_PARAM | Трубы, Воздуховоды, Фитинги, Арматура |

**Конвертация единиц (фиксированная):**
- Длина/Ширина/Высота/Толщина/Диаметр: feet × 304.8 → **мм** (всегда в мм)
- Площадь: feet² × 0.092903 → **м²**
- Объём: feet³ × 0.0283168 → **м³**

**Логика работы:**
1. **Общестроительные категории** (Стены, Перекрытия, Крыши, Потолки, Фундаменты, Колонны, Балки):
   - Для `Length/Width/Height/Thickness` приоритетный расчёт по `BoundingBoxXYZ`:
     - `Длина` = максимум из горизонтальных размеров
     - `Ширина` = минимум из горизонтальных размеров
     - `Высота` = вертикальный размер
     - `Толщина` = минимальный из всех трёх размеров
   - Если BoundingBox недоступен — fallback к системным параметрам

2. **Остальные категории**:
   - Значения берутся из соответствующих системных параметров
   - Порядок источников: первый с непустым значением используется

3. **Обработка отсутствующих значений**:
   - Если ни один источник не даёт значение → параметр остаётся незаполненным
   - Для общестроительных элементов это редкость (BoundingBox есть почти всегда)

4. **Оптимизация**:
   - Кэширование маппинга `категория → список BIP` для каждого параметра
   - Ускорение работы на больших моделях (сотни тысяч элементов)

5. **Безопасность транзакций**:
   - Если `doc.IsModifiable == True` — работает без открытия транзакции
   - Иначе — создаёт и коммитит свою транзакцию
   - При ошибке корректно откатывает изменения

---

### 8. fill_section_from_coordination.py ⭐
...
### 9. swap_level_names.py
Обмен значений между параметрами уровня `ДГП_Имя уровня` и `Имя`.

**Целевые параметры:**
- `ДГП_Имя уровня`
- `Имя`

**Источник:** Взаимный обмен значений между этими двумя параметрами.

**Логика:**
- Собирает все элементы категории `OST_Levels`
- Для каждого уровня меняет местами содержимое:
  - `ДГП_Имя уровня` <- старое `Имя`
  - `Имя` <- старое `ДГП_Имя уровня`
- Если оба значения пустые — уровень пропускается
- Поддерживает пакетный прогресс через `progress_callback`
- Возвращает статистику `updated/skipped/reasons` в стандартном формате batch-скриптов
...
5. **Revit-version-safe BuiltInParameter (BIP):**
   - Пакетный запуск загружает скрипты через `imp.load_source(...)` - исключения при импорте останавливают выполнение скрипта
   - Избегайте использования `BuiltInParameter.X` в константах на уровне модуля; предпочтительно хранить имена членов enum как строки
   - Разрешайте во время выполнения с помощью `Enum.IsDefined` + `Enum.Parse` (см. `auto_navis_export_script.py`)
   - Пример безопасного подхода реализован в `fill_dimensions.py`
