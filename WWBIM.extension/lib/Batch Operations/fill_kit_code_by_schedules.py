...
def IsValidSheetKitCodeValue(value, kit_mode):
    if not value:
        return False
    if kit_mode == "EngineeringSchedules":
        return ".СО" in value
    return IsValidKitCodeValue(value)


def NormalizeKitCodeValue(value, kit_mode):
    if not value:
        return value
    if kit_mode == "EngineeringSchedules":
        marker = ".СО"
...
def GetSheetsWithValidKitCodes(doc, sheet_param_name, kit_mode):
    ...
    sheets_to_check = []
    if kit_mode in ("Schedules", "EngineeringSchedules"):
    ...
    for sheet in sheets_to_check:
        ...
        if param and param.HasValue:
            try:
                ...
                if IsValidSheetKitCodeValue(value, kit_mode):
...
def FillKitCodes_Schedules(doc, sheet_param_name, kit_mode, progress_callback=None):
    schedule_instances = (
        FilteredElementCollector(doc).OfClass(ScheduleSheetInstance).ToElements()
    )
    ...
    for schedule_inst in schedule_instances:
        ...
        if not IsValidSheetKitCodeValue(kit_code, kit_mode):
            continue

        kit_code = NormalizeKitCodeValue(kit_code, kit_mode)
    ...
    try:
        ...
        if kit_mode in ("Schedules", "EngineeringSchedules"):
            fill_result = FillKitCodes_Schedules(
                doc, sheet_param_name, kit_mode, progress_callback
            )
        else:
