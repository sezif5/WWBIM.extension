...
# --- Фолбэк вместо dosymep_libs.bim4everyone ---
try:
    # если библиотека есть — используем её
    from dosymep_libs.bim4everyone import notification, log_plugin
except Exception:
    # минимальные замены, чтобы скрипт работал без dosymep_libs
    from pyrevit import forms as _forms
    from pyrevit import script as _script

    class _DummyLogger(object):
        def __init__(self):
            self._log = _script.get_logger()
    ...
    def log_plugin(*dargs, **dkwargs):
        # сохраняем сигнатуру: def script_execute(plugin_logger)
        def _decorator(func):
            def _wrapper(*args, **kwargs):
                return func(_DummyLogger(), *args, **kwargs)
            return _wrapper
        return _decorator

    def notification(*dargs, **dkwargs):
        # перехватываем исключения и показываем alert
        def _decorator(func):
            def _wrapper(*args, **kwargs):
                try:
                    ...
                    import traceback
                    _script.get_logger().error(traceback.format_exc())
                    try:
                        _forms.alert(u"Ошибка выполнения:\n{}".format(ex), exitscript=True)
                    except Exception:
                        raise
            return _wrapper
        return _decorator
