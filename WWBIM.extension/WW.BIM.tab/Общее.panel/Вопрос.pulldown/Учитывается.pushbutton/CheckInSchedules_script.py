...
def find_schedules_for_element(element, only_on_sheets=False, schedule_sheet_map=None):
    ...
    with forms.ProgressBar(
        ...
        for idx, schedule in enumerate(schedules):
            ...
            if pb.cancelled:
                output.print_md(u"⚠️ **Проверка отменена пользователем**")
                break
...
def main():
    ...
    # Сначала получаем общее количество спецификаций для определения необходимости диалога
    all_schedules_count = FilteredElementCollector(doc).OfClass(ViewSchedule).GetElementCount()
    only_on_sheets = False
    
    if all_schedules_count > 50:
        result = forms.alert(
